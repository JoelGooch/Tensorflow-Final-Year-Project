...
class CNNApp(QMainWindow, design.Ui_MainWindow):
	...
	def __init__(self):  
		...
		self.txtLoadModel.setText('C:/Users/Joel Gooch/Desktop/Final Year/PRCO304/Tensorflow-Final-Year-Project/Models/Testingagain.xml')
	...
	def trainButtonClicked(self):
		try:
			...
			worker = Worker(self.currentDataSet(), num_epochs, batch_size, learning_rate, optimizer, save_path, save_interval, model_path)
			thread = QThread()
	...
	def createConvLayerButtonClicked(self):
		...
		try:
			...
			if self.chkConvPadding.currentIndex() == 0:
				padding = 'SAME'
			...
			layer = l.ConvLayer(conv_layer_name, conv_kernel_size, conv_stride, act_function, num_output_filters, padding, normalize, dropout, keep_rate)
	...
	def createDenseLayerButtonClicked(self):
		...
		try:
			...
			layer = l.DenseLayer(dense_layer_name, act_function, num_output_nodes, normalize, dropout, keep_rate)
			self.newModel.append(layer)
	...
	def createOutputLayerButtonClicked(self):
		try:
			...
			layer = l.OutputLayer(output_layer_name, act_function)
			self.newModel.append(layer)
