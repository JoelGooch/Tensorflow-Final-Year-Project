...
def evaluate_accuracy(predictions, labels, pickle_file, test_or_train, write_to_csv=False):
	...
	if write_to_csv == True:
		...
		file = open('/tmp/AFLW/Yaw/log/' + test_or_train + '_person' + str(pickle_file) + '_yaw' + str(now.hour) + str(now.minute) + str(now.second) + '.csv', 'a')
...
def main():
	...
	pickle_directory = 'C:/Users/Joel Gooch/Desktop/Final Year/PRCO304/data/AFLW/yaw/afw_f'

	# there are 5 files (5 people), each uses jack knife cross validation meaning in each file a different person is used for the testing set
	# essentially this makes 55 separate networks, the final accuracy terms are the mean across all networks
	for pickle_file in range(1, 6):

		print('\nRunning with Person {0} as the testing set\n'.format(str(pickle_file)))

		current_pickle = pickle_directory + str(pickle_file) + '.pickle'
		...
		with graph.as_default():
			...
			with tf.Session(graph=graph) as session:
				...
				log_path = '/tmp/AFLW/Yaw/log/person' + str(pickle_file) + '_' + str(curr_time.hour) + str(curr_time.minute) + str(curr_time.second)
				...
				save_path = '/tmp/AFLW/Yaw/checkpoints/person' + str(pickle_file) + '/'
				# if folder does not exist, create it
				...
				file = open('/tmp/AFLW/Yaw/log/batchstats_person' + str(pickle_file) + '_yaw.csv', 'a')
